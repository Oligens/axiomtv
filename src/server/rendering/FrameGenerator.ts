...
export class FrameGenerator {
  ...
  async generateFrame(data: FrameData, framesDir: string): Promise<string> {
    ...
    if (background && await fs.pathExists(background)) {
      image = sharp(background).resize(this.width, this.height, { fit: 'cover' });
    } else {
      image = sharp({
        create: {
          width: this.width,
    ...
    const text = escapeXml(data.segment.text);
    const svg = `<svg width="${this.width}" height="${this.height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="80" y="${this.height - 95}" fill="#fff" font-size="42" font-family="Arial, sans-serif">${text.slice(0, 140)}</text>
  ...
  async cleanup(): Promise<void> {
    // Frames are owned by SciFiRenderer and removed there after assembly.
  }
}
