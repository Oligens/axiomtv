...
export class SciFiRenderer {
  ...
  async initialize(): Promise<void> {
    await Promise.all([fs.ensureDir(path.join(this.outputDir, 'frames')), fs.ensureDir(path.join(this.outputDir, 'audio')), fs.ensureDir(path.join(this.outputDir, 'logs'))]);
  }
  ...
  async render(script: ScriptSegment[]): Promise<string> {
    ...
    try {
      ...
      if (this.config.cinematic !== false) {
        ...
        await this.cinematicProcessor.process({ input: assembledPath, output: outputPath, fps: this.config.fps, format: this.config.outputFormat, letterbox: this.config.letterbox !== false, intensity: this.config.cinematicIntensity ?? 0.65 });
  ...
  private generateFrameData(script: ScriptSegment[], phonemes: Phoneme[], wordTimings: WordTiming[], totalFrames: number, audioDuration: number, shotPlan: ReturnType<CinemaDirector['plan']>): FrameData[] {
...
function findTiming<T extends { start: number; end: number }>(items: T[], timestamp: number): T | null { let low = 0; let high = items.length - 1; while (low <= high) { const mid = (low + high) >> 1; const item = items[mid]; if (timestamp < item.start) high = mid - 1; else if (timestamp >= item.end) low = mid + 1; else return item; } return null; }
