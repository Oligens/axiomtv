...
import type { Phoneme, WordTiming } from './types';
...
export class AudioEngine {
  constructor(private readonly outputDir: string, private readonly apiKey: string, private readonly voiceId: string) {}

  async generateAudioWithElevenLabs(text: string): Promise<{ audioPath: string; metadataPath: string; wordTimings: WordTiming[]; phonemes: Phoneme[] }> {
    ...
    const wordTimings = buildWordTimings(payload.alignment);
    const phonemes = buildCharacterTimings(payload.alignment);
    await fs.writeJson(metadataPath, { wordTimings, phonemes, generatedAt: new Date().toISOString() }, { spaces: 2 });
    return { audioPath, metadataPath, wordTimings, phonemes };
  }

  async getAudioDuration(audioPath: string): Promise<number> {
    const binary = process.env.FFPROBE_PATH || 'ffprobe';
    return new Promise((resolve, reject) => {
      const child = spawn(binary, ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', audioPath]);
      let out = ''; let err = '';
...
function buildCharacterTimings(alignment?: Alignment): Phoneme[] {
  const chars = alignment?.characters ?? [];
  const starts = alignment?.character_start_times_seconds ?? [];
  const ends = alignment?.character_end_times_seconds ?? [];
  return chars.map((value, i) => ({ start: starts[i] ?? 0, end: ends[i] ?? 0, value })).filter((p) => p.end > p.start);
}

function buildWordTimings(alignment?: Alignment): WordTiming[] {
  const chars = alignment?.characters ?? [];
  const starts = alignment?.character_start_times_seconds ?? [];
  const ends = alignment?.character_end_times_seconds ?? [];
  const result: WordTiming[] = [];
  let word = ''; let start = 0; let end = 0;
  for (let i = 0; i < chars.length; i++) {
    const ch = chars[i];
    if (!word && /\S/.test(ch)) start = starts[i] ?? 0;
    if (/\s/.test(ch)) { if (word) result.push({ word, start, end }); word = ''; continue; }
    word += ch; end = ends[i] ?? end;
  }
