...
function checkBinary(binary: string, label: string): Promise<void> {
  return new Promise((resolve, reject) => {
    const child = spawn(binary, ['-version'], { stdio: 'ignore' });
    child.on('error', () => reject(new Error(`${label} not found. Set ${label === 'FFmpeg' ? 'FFMPEG_PATH' : 'FFPROBE_PATH'} or install it on the server.`)));
