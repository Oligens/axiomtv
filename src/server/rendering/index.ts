...
export async function runRenderer(argv = process.argv.slice(2)): Promise<string | void> {
  ...
  const script = await loadScript(argv); const apiKey = process.env.ELEVENLABS_API_KEY; const voiceId = process.env.ELEVENLABS_VOICE_ID;
  ...
  try { return await renderer.render(script); } finally { process.removeListener('SIGINT', stop); process.removeListener('SIGTERM', stop); }
}
