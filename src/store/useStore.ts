...
export const useStore = create<Store>()(
  persist(
    (set, get) => ({
      ...
      consumeAgweVideo: () => set((s) => ({ agwePack: s.agwePack ? { videosLeft: Math.max(0, s.agwePack.videosLeft - 1) } : null })),
    ...
    {
      name: "axiom-tv-store",
      partialize: (s) => ({ user: s.user, authToken: s.authToken, publications: s.publications, paymentMethods: s.paymentMethods, agwePack: s.agwePack, introMeta: s.introMeta }),
    }
  )
);
